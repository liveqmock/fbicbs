 ...
 */
@ManagedBean(name="actedition")
@ViewScoped
public class ActEdition {
    @ManagedProperty("#{ibatisManager}")
    IbatisManager ibatisManager;
    private static final Log logger = LogFactory.getLog(ActEdition.class);
...
//                context.addMessage(null,new FacesMessage(FacesMessage.SEVERITY_ERROR,"该账户已不存在！","failed"));
                return null;
            } else if (!actsts1.equals(" ")) {
                MessageUtil.addInfo("该账户不可修改");
//                context.addMessage(null,new FacesMessage(FacesMessage.SEVERITY_WARN,"该账户不可修改！","warn"));
                return null;
            }
...
//            context.addMessage(null,new FacesMessage(FacesMessage.SEVERITY_ERROR,"不能进入编辑页面！","failed"));
            return null;
        }
    }
